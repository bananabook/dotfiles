...
Sound2(){
	...
	if [[ ! -z $NOTMUTED ]] ; then
		VOL=$(awk -F"[][]" '/dB/ { print $2 }' <(amixer -D pulse sget Master|grep %]) | sed 's/%//g')
		if [ $VOL -ge 85 ] ; then
			echo -e "\uf028${VOL}%"
		elif [ $VOL -ge 50 ] ; then
			echo -e "\uf027${VOL}%"
...
while $LEMONBAR; do
	echo  "%{Sl}%{F#C5C8C6}%{B#1D1F21}%{l} $(Out)%{F#8C9440} $(Uptime-p)%{F#C5C8C6}   $(Ram)%{r} $(Sound2) $(Cal) $(Battery)$(Battery_State) %{F#CC6666}$(Clock)"
	sleep 5s
done
